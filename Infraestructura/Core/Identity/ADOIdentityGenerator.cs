...
namespace Infraestructura.Core.Identity
{
    ...
    {
        private const string ConnectionString = "conectionDataBase";
        ...
        {
            var connectionString = DataBaseConnectionStrings.ObtenerConexion("conectionDataBase");

            string siguienteCorrelativo = string.Empty;
            if (string.IsNullOrWhiteSpace(connectionString)) throw new ApplicationException(_connectionExceptionMessage);

            using (var conexionSp = new SqlConnection(connectionString))
            {
                using (var comandoSp = new SqlCommand("Comunes.ObtenerCorrelativo", conexionSp))
                {
                    ...
                    comandoSp.Parameters.Add("@SiguienteCorrelativo", SqlDbType.VarChar, 50).Direction = ParameterDirection.Output;
